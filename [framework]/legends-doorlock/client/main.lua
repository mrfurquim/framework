            ...
            if distance < maxDistance then
                if distance < 1.5 then
                    ...
                    if PromptHasHoldModeCompleted(lockPrompt) and CoolDown < 1 then
                        ...
                        TriggerServerEvent("legends-doorlock:updatedoorsv", k, state)
                    end
                end
            end
...
RegisterNetEvent('legends-doorlock:changedoor')
AddEventHandler('legends-doorlock:changedoor', function(doorID, state)
    local ped = PlayerPedId()
    ...
    if not IsEntityPlayingAnim(ped, "script_common@jail_cell@unlock@key", "action", 3) then
        ...
        Wait(250)
        TriggerServerEvent('legends-doorlock:updateState', doorID, state, function(cb) end)
        Wait(1500)
...
RegisterNetEvent('legends-doorlock:setState')
AddEventHandler('legends-doorlock:setState', function(doorID, state)
