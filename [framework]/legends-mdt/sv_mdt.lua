		...
		job_access = true
				exports.oxmysql:fetch("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 6) sub ORDER BY `id` DESC", {}, function(reports)
					...
					exports.oxmysql:fetch("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 6) sub ORDER BY `id` DESC", {}, function(warrants)
						...
						TriggerClientEvent('legends-mdt:toggleVisibilty', _source, reports, warrants, officername, job, jobgrade, note)
					end)
				end)
			end)
            end
...
RegisterServerEvent("legends-mdt:getOffensesAndOfficer")
AddEventHandler("legends-mdt:getOffensesAndOfficer", function()
	...
	exports.oxmysql:fetch('SELECT * FROM fine_types', {}, function(fines)
		...
		TriggerClientEvent("legends-mdt:returnOffensesAndOfficer", usource, charges, officername)
	end)
end)


RegisterServerEvent("legends-mdt:performOffenderSearch")
AddEventHandler("legends-mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:query("SELECT * FROM `players` WHERE `charinfo` LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
		...
		TriggerClientEvent("legends-mdt:returnOffenderSearchResults", usource, matches)
	end)
end)
...
RegisterServerEvent("legends-mdt:getOffenderDetails")
AddEventHandler("legends-mdt:getOffenderDetails", function(offender)
	local usource = source
    ...
    exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {offender.id}, function(result)
        ...
        exports.oxmysql:fetch('SELECT * FROM `user_convictions` WHERE `char_id` = ?', {offender.id}, function(convictions)
            ...
            exports.oxmysql:fetch('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {offender.id}, function(warrants)
				...
				TriggerClientEvent("legends-mdt:returnOffenderDetails", usource, offender)
            end)
        end)
    end)
end)
...
RegisterServerEvent("legends-mdt:getOffenderDetailsById")
AddEventHandler("legends-mdt:getOffenderDetailsById", function(char_id)
    local usource = source
	print(char_id)
    ...
    exports.oxmysql:execute('SELECT * FROM `players` WHERE `id` = ?', {char_id}, function(result)
        ...
        if not offender then
            TriggerClientEvent("legends-mdt:closeModal", usource)
            TriggerClientEvent("legends-mdt:sendNotification", usource, "This person no longer exists.")
            return
        end
...
RegisterServerEvent("legends-mdt:saveOffenderChanges")
AddEventHandler("legends-mdt:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	...
	exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {id}, function(result)
		...
		TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['1'])
	end)
end)

RegisterServerEvent("legends-mdt:saveReportChanges")
AddEventHandler("legends-mdt:saveReportChanges", function(data)
	exports.oxmysql:execute('UPDATE `mdt_reports` SET `title` = ?, `incident` = ? WHERE `id` = ?', {data.id, data.title, data.incident})
	TriggerClientEvent("legends-mdt:sendNotification", source, Config.Notify['2'])
end)

RegisterServerEvent("legends-mdt:deleteReport")
AddEventHandler("legends-mdt:deleteReport", function(id)
	exports.oxmysql:execute('DELETE FROM `mdt_reports` WHERE `id` = ?', {id})
	TriggerClientEvent("legends-mdt:sendNotification", source, Config.Notify['3'])
end)

RegisterServerEvent("legends-mdt:deleteNote")
AddEventHandler("legends-mdt:deleteNote", function(id)
	exports.oxmysql:execute('DELETE FROM `mdt_telegrams` WHERE `id` = ?', {id})
	TriggerClientEvent("legends-mdt:sendNotification", source, Config.Notify['9'])
end)

RegisterServerEvent("legends-mdt:submitNewReport")
AddEventHandler("legends-mdt:submitNewReport", function(data)
	local usource = source
	local tablex = {}
    local _source = source
	local xPlayer = RSGCore.Functions.GetPlayer(usource)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?)', {data.char_id, data.title, data.incident, charges, officername, data.name, data.date,}, function(id)
		TriggerEvent("legends-mdt:getReportDetailsById", id, usource)
		TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['4'])
	end)
...
RegisterServerEvent("legends-mdt:submitNote")
AddEventHandler("legends-mdt:submitNote", function(data)
	local usource = source
	local tablex = {}
    local _source = source
	local xPlayer = RSGCore.Functions.GetPlayer(usource)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_telegrams` ( `title`, `incident`, `author`, `date`) VALUES (?, ?, ?, ?)', {data.title, data.note, officername, data.date,}, function(id)
		TriggerEvent("legends-mdt:getNoteDetailsById", id, usource)
		TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['8'])
	end)
end)

RegisterServerEvent("legends-mdt:performReportSearch")
AddEventHandler("legends-mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:fetch("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("legends-mdt:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("legends-mdt:getWarrants")
AddEventHandler("legends-mdt:getWarrants", function()
	local usource = source
	exports.oxmysql:fetch("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("legends-mdt:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("legends-mdt:submitNewWarrant")
AddEventHandler("legends-mdt:submitNewWarrant", function(data)
	local usource = source
	local tablex = {}
    local _source = source
	local xPlayer = RSGCore.Functions.GetPlayer(usource)
	...
	exports.oxmysql:insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', {data.name, data.char_id, data.report_id, data.report_title, data.charges, data.date, data.expire, data.notes, data.author}, function()
		TriggerClientEvent("legends-mdt:completedWarrantAction", usource)
		TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['5'])
	end)
end)

RegisterServerEvent("legends-mdt:deleteWarrant")
AddEventHandler("legends-mdt:deleteWarrant", function(id)
	local usource = source
	exports.oxmysql:execute('DELETE FROM `mdt_warrants` WHERE `id` = ?', {id}, function()
		TriggerClientEvent("legends-mdt:completedWarrantAction", usource)
	end)
	TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['6'])
end)

RegisterServerEvent("legends-mdt:getReportDetailsById")
AddEventHandler("legends-mdt:getReportDetailsById", function(query, _source)
	...
	exports.oxmysql:fetch("SELECT * FROM `mdt_reports` WHERE `id` = ?", {query}, function(result)
		if result and result[1] then
			...
			TriggerClientEvent("legends-mdt:returnReportDetails", usource, result[1])
		else
			TriggerClientEvent("legends-mdt:closeModal", usource)
			TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['7'])
		end
	end)
end)

RegisterServerEvent("legends-mdt:getNoteDetailsById")
AddEventHandler("legends-mdt:getNoteDetailsById", function(query, _source)
	if _source then source = _source end
	local usource = source
	exports.oxmysql:fetch("SELECT * FROM `mdt_telegrams` WHERE `id` = ?", {query}, function(result)
		if result and result[1] then
			TriggerClientEvent("legends-mdt:returnNoteDetails", usource, result[1])
		else
			TriggerClientEvent("legends-mdt:closeModal", usource)
			TriggerClientEvent("legends-mdt:sendNotification", usource, Config.Notify['8'])
		end
