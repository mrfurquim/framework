...
local MainMenus = {
    ...
    ["save"] = function()
        TriggerServerEvent("legends-appearance:SetPlayerBucket" , 0)
        EndCharacterCreatorCam()
        MenuData.CloseAll()
        LoadedComponents = CreatorCache
        TriggerServerEvent("legends-appearance:SaveSkin", CreatorCache)
    end
}
...
RegisterNetEvent('legends-appearance:ApplySkin')
AddEventHandler('legends-appearance:ApplySkin', function(SkinData, Target, ClothesData)
    ...
    CreateThread(function()
        ...
        if _Target == PlayerPedId() then
            ...
            TriggerEvent("legends-clothes:ApplyClothes", ClothesData, _Target)
...
RegisterNetEvent('legends-appearance:OpenCreator')
AddEventHandler('legends-appearance:OpenCreator', function()
    StartCreator()
end)

RegisterNetEvent('legends-appearance:LoadSkinClient')
AddEventHandler('legends-appearance:LoadSkinClient', function()
    if isLoggedIn then
...
function StartCreator()
    TriggerServerEvent("legends-appearance:SetPlayerBucket" , BucketId)
    Wait(1)
    for i, m in pairs(overlay_all_layers) do
        overlay_all_layers[i] =
        {name = m.name, visibility = 0, tx_id = 1, tx_normal = 0, tx_material = 0, tx_color_type = 0, tx_opacity = 1.0, tx_unk = 0, palette = 0, palette_color_primary = 0, palette_color_secondary = 0, palette_color_tertiary = 0, var = 0, opacity = 0.0}
