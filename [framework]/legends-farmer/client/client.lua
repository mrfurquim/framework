...
function DestroyPlant()
    ...
    TriggerServerEvent('legends-farmer:server:plantHasBeenHarvested', plant.id)
    TaskStartScenarioInPlace(ped, `WORLD_HUMAN_CROUCH_INSPECT`, 0, true)
    Wait(5000)
    ClearPedTasks(ped)
    SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
    FreezeEntityPosition(ped, false)
    TriggerServerEvent('legends-farmer:server:destroyPlant', plant.id)
    isDoingAction = false
    canHarvest = true
end
...
function HarvestPlant()
    ...
    SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
    FreezeEntityPosition(ped, false)
    TriggerServerEvent('legends-farmer:server:harvestPlant', plant.id)
    isDoingAction = false
    canHarvest = true
end
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) < 1.3 and not isDoingAction and not v.beingHarvested and not IsPedInAnyVehicle(PlayerPedId(), false) then
                if PlayerJob.name == 'police' then
                    ...
                    if v.growth < 100 then
                        ...
                        if IsControlJustPressed(0, RSGCore.Shared.Keybinds['G']) then
                            if v.id == plant.id then
                                TriggerEvent('legends-farmer:client:waterPlant')
                            end
                        elseif IsControlJustPressed(0, RSGCore.Shared.Keybinds['J']) then
                            if v.id == plant.id then
                                TriggerEvent('legends-farmer:client:feedPlant')
                            end
                        end
                    else
...
-- remove plant object
RegisterNetEvent('legends-farmer:client:removePlantObject')
AddEventHandler('legends-farmer:client:removePlantObject', function(plant)
...
-- water plants
RegisterNetEvent('legends-farmer:client:waterPlant')
AddEventHandler('legends-farmer:client:waterPlant', function()
    local entity = nil
    local plant = GetClosestPlant()
    local ped = PlayerPedId()
    isDoingAction = true
    ...
    if hasItem1 and hasItem2 then
        ...
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        FreezeEntityPosition(ped, false)
        TriggerServerEvent('legends-farmer:server:waterPlant', plant.id)
        isDoingAction = false
...
-- feed plants
RegisterNetEvent('legends-farmer:client:feedPlant')
AddEventHandler('legends-farmer:client:feedPlant', function()
    local entity = nil
    local plant = GetClosestPlant()
    local ped = PlayerPedId()
    isDoingAction = true
    ...
    if hasItem1 and hasItem2 then
        ...
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        FreezeEntityPosition(ped, false)
        TriggerServerEvent('legends-farmer:server:feedPlant', plant.id)
        isDoingAction = false
...
RegisterNetEvent('legends-farmer:client:updatePlantData')
AddEventHandler('legends-farmer:client:updatePlantData', function(data)
    Config.FarmPlants = data
end)

-- Plant Seeds
RegisterNetEvent('legends-farmer:client:plantNewSeed')
AddEventHandler('legends-farmer:client:plantNewSeed', function(planttype, pHash, seed)
    ...
    -- Seed Based Farm Zone
    if Config.UseSeedBasedZones == true then
        seedBasedZones = true
    end
    ...
    if CanPlantSeedHere(pos) and not IsPedInAnyVehicle(PlayerPedId(), false) and not isBusy then
        ...
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        FreezeEntityPosition(ped, false)
        TriggerServerEvent('legends-farmer:server:removeitem', seed, 1)
        TriggerServerEvent('legends-farmer:server:plantNewSeed', planttype, pos, pHash)
        isBusy = false
...
Citizen.CreateThread(function()
    for farmshop, v in pairs(Config.FarmShopLocations) do
        exports['legends-core']:createPrompt(v.name, v.coords, 0xF3830D8E, Lang:t('menu.open') .. v.name, {
            type = 'client',
            event = 'legends-farmer:client:OpenFarmShop',
        })
...
RegisterNetEvent('legends-farmer:client:OpenFarmShop')
AddEventHandler('legends-farmer:client:OpenFarmShop', function()
    local ShopItems = {}
