...
RSGCore.Functions.CreateUseableItem("cornseed", function(source, item)
    ...
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'corn', 'CRP_CORNSTALKS_AB_SIM', 'cornseed')
end)
...
RSGCore.Functions.CreateUseableItem("sugarseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'sugar', 'CRP_SUGARCANE_AC_SIM', 'sugarseed')
end)
...
RSGCore.Functions.CreateUseableItem("tobaccoseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'tobacco', 'CRP_TOBACCOPLANT_AC_SIM', 'tobaccoseed')
end)
...
RSGCore.Functions.CreateUseableItem("carrotseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'carrot', 'CRP_CARROTS_AA_SIM', 'carrotseed')
end)
...
RSGCore.Functions.CreateUseableItem("tomatoseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'tomato', 'CRP_TOMATOES_AA_SIM', 'tomatoseed')
end)
...
RSGCore.Functions.CreateUseableItem("broccoliseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'broccoli', 'crp_broccoli_aa_sim', 'broccoliseed')
end)
...
RSGCore.Functions.CreateUseableItem("potatoseed", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-farmer:client:plantNewSeed', src, 'potato', 'crp_potato_aa_sim', 'potatoseed')
end)

-----------------------------------------------------------------------

-- remove seed item
RegisterServerEvent('legends-farmer:server:removeitem')
AddEventHandler('legends-farmer:server:removeitem', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
...
Citizen.CreateThread(function()
    while true do
        ...
        if PlantsLoaded then
            TriggerClientEvent('legends-farmer:client:updatePlantData', -1, Config.FarmPlants)
        end
    end
end)

Citizen.CreateThread(function()
    TriggerEvent('legends-farmer:server:getPlants')
    PlantsLoaded = true
end)

RegisterServerEvent('legends-farmer:server:savePlant')
AddEventHandler('legends-farmer:server:savePlant', function(data, plantId, citizenid)
...
-- plant seed
RegisterServerEvent('legends-farmer:server:plantNewSeed')
AddEventHandler('legends-farmer:server:plantNewSeed', function(planttype, location, hash)
    local src = source
    ...
    if PlantCount >= Config.MaxPlantCount then
        ...
        TriggerEvent('legends-farmer:server:savePlant', SeedData, plantId, citizenid)
        TriggerEvent('legends-farmer:server:updatePlants')
    end
end)

-- check plant
RegisterServerEvent('legends-farmer:server:plantHasBeenHarvested')
AddEventHandler('legends-farmer:server:plantHasBeenHarvested', function(plantId)
    for k, v in pairs(Config.FarmPlants) do
...
RegisterServerEvent('legends-farmer:server:destroyPlant')
AddEventHandler('legends-farmer:server:destroyPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.FarmPlants) do
        if v.id == plantId then
    ...
    TriggerClientEvent('legends-farmer:client:removePlantObject', -1, plantId)
    TriggerEvent('legends-farmer:server:PlantRemoved', plantId)
    TriggerEvent('legends-farmer:server:updatePlants')
    TriggerClientEvent('RSGCore:Notify', src, Lang:t('success.you_distroyed_the_plant'), 'success')
end)

-- harvest plant and give reward
RegisterServerEvent('legends-farmer:server:harvestPlant')
AddEventHandler('legends-farmer:server:harvestPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local poorAmount = 0
    local goodAmount = 0
...
RegisterServerEvent('legends-farmer:server:updatePlants')
AddEventHandler('legends-farmer:server:updatePlants', function()
    local src = source
    TriggerClientEvent('legends-farmer:client:updatePlantData', src, Config.FarmPlants)
end)

-- water plant
RegisterServerEvent('legends-farmer:server:waterPlant')
AddEventHandler('legends-farmer:server:waterPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.FarmPlants) do
        if v.id == plantId then
...
-- feed plant
RegisterServerEvent('legends-farmer:server:feedPlant')
AddEventHandler('legends-farmer:server:feedPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.FarmPlants) do
        if v.id == plantId then
...
-- update plant
RegisterServerEvent('legends-farmer:server:updateFarmPlants')
AddEventHandler('legends-farmer:server:updateFarmPlants', function(id, data)
...
-- remove plant
RegisterServerEvent('legends-farmer:server:PlantRemoved')
AddEventHandler('legends-farmer:server:PlantRemoved', function(plantId)
...
-- get plant
RegisterServerEvent('legends-farmer:server:getPlants')
AddEventHandler('legends-farmer:server:getPlants', function()
    local data = {}
    local result = MySQL.query.await('SELECT * FROM player_plants')
    if result[1] then
        for i = 1, #result do
        ...
        for i = 1, #Config.FarmPlants do
            if Config.FarmPlants[i].growth < 100 then
                ...
                if currenttime > untildead then
                    ...
                    TriggerEvent('legends-farmer:server:PlantRemoved', deadid)
                end
            end
            TriggerEvent('legends-farmer:server:updateFarmPlants', Config.FarmPlants[i].id, Config.FarmPlants[i])
        end
        TriggerEvent('legends-farmer:server:updatePlants')
    end
end)
...
RegisterServerEvent('legends-farmer:server:giveitem')
AddEventHandler('legends-farmer:server:giveitem', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
