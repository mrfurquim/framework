...
AddEventHandler('RSGCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    RSGCore.Functions.TriggerCallback('legends-scoreboard:server:GetConfig', function(config) Config.IllegalActions = config end)
end)

RegisterNetEvent('legends-scoreboard:client:SetActivityBusy')
AddEventHandler('legends-scoreboard:client:SetActivityBusy', function(activity, busy)
    ...
    while true do
        ...
        if IsControlJustReleased(0, RSGCore.Shared.Keybinds['PGDN']) and IsInputDisabled(0) then -- PAGEDOWN
            if not scoreboardOpen then
                RSGCore.Functions.TriggerCallback('legends-scoreboard:server:GetPlayersArrays', function(playerList)
                    RSGCore.Functions.TriggerCallback('legends-scoreboard:server:GetActivity', function(police, medic)
                        RSGCore.Functions.TriggerCallback("legends-scoreboard:server:GetCurrentPlayers", function(Players)
