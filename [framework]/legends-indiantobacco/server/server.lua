...
RSGCore.Functions.CreateUseableItem("indtobaccoseed", function(source, item)
    ...
    TriggerClientEvent('legends-indiantobacco:client:plantNewSeed', src, 'indtobacco')
...
RSGCore.Functions.CreateUseableItem("indiancigar", function(source, item)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    TriggerClientEvent('legends-indiantobacco:client:SmokeIndian', src)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if PlantsLoaded then
            TriggerClientEvent('legends-indiantobacco:client:updatePlantData', -1, Config.Plants)
        end
    end
end)

Citizen.CreateThread(function()
    TriggerEvent('legends-indiantobacco:server:getPlants')
    PlantsLoaded = true
end)

RegisterServerEvent('legends-indiantobacco:server:savePlant')
AddEventHandler('legends-indiantobacco:server:savePlant', function(data, plantId)
...
-- give seed
RegisterServerEvent('legends-indiantobacco:server:giveSeed')
AddEventHandler('legends-indiantobacco:server:giveSeed', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
...
-- plant seed
RegisterServerEvent('legends-indiantobacco:server:plantNewSeed')
AddEventHandler('legends-indiantobacco:server:plantNewSeed', function(type, location)
    local src = source
    ...
    if PlantCount >= Config.MaxPlantCount then
        ...
        TriggerEvent('legends-indiantobacco:server:savePlant', SeedData, plantId)
        TriggerEvent('legends-indiantobacco:server:updatePlants')
    end
end)

-- check plant
RegisterServerEvent('legends-indiantobacco:server:plantHasBeenHarvested')
AddEventHandler('legends-indiantobacco:server:plantHasBeenHarvested', function(plantId)
    for k, v in pairs(Config.Plants) do
...
RegisterServerEvent('legends-indiantobacco:server:destroyPlant')
AddEventHandler('legends-indiantobacco:server:destroyPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.Plants) do
        if v.id == plantId then
    ...
    TriggerClientEvent('legends-indiantobacco:client:removePlantObject', src, plantId)
    TriggerEvent('legends-indiantobacco:server:PlantRemoved', plantId)
    TriggerEvent('legends-indiantobacco:server:updatePlants')
    TriggerClientEvent('RSGCore:Notify', src, 'you distroyed the plant', 'success')
end)

-- harvest plant
RegisterServerEvent('legends-indiantobacco:server:harvestPlant')
AddEventHandler('legends-indiantobacco:server:harvestPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local amount
    local label
    ...
    if hasFound then        
        if poorQuality then
            ...
            Wait(5000)
            TriggerEvent('legends-indiantobacco:server:dealerrep', src)
...
RegisterServerEvent('legends-indiantobacco:server:updatePlants')
AddEventHandler('legends-indiantobacco:server:updatePlants', function()
    local src = source
    TriggerClientEvent('legends-indiantobacco:client:updatePlantData', src, Config.Plants)
end)

-- water plant
RegisterServerEvent('legends-indiantobacco:server:waterPlant')
AddEventHandler('legends-indiantobacco:server:waterPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.Plants) do
        if v.id == plantId then
...
-- feed plant
RegisterServerEvent('legends-indiantobacco:server:feedPlant')
AddEventHandler('legends-indiantobacco:server:feedPlant', function(plantId)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    for k, v in pairs(Config.Plants) do
        if v.id == plantId then
...
-- update plant
RegisterServerEvent('legends-indiantobacco:server:updateIndianPlants')
AddEventHandler('legends-indiantobacco:server:updateIndianPlants', function(id, data)
...
-- remove plant
RegisterServerEvent('legends-indiantobacco:server:PlantRemoved')
AddEventHandler('legends-indiantobacco:server:PlantRemoved', function(plantId)
...
-- get plant
RegisterServerEvent('legends-indiantobacco:server:getPlants')
AddEventHandler('legends-indiantobacco:server:getPlants', function()
    local data = {}
    local result = MySQL.query.await('SELECT * FROM indian_plants')
    if result[1] then
        for i = 1, #result do
        ...
        for i = 1, #Config.Plants do
            if Config.Plants[i].growth < 100 then
                ...
                if currenttime > untildead then
                    ...
                    TriggerEvent('legends-indiantobacco:server:PlantRemoved', deadid)
                end
            end
            TriggerEvent('legends-indiantobacco:server:updateIndianPlants', Config.Plants[i].id, Config.Plants[i])
        end
        TriggerEvent('legends-indiantobacco:server:updatePlants')
    end
end)
...
RegisterServerEvent('legends-indiantobacco:server:dealerrep')
AddEventHandler('legends-indiantobacco:server:dealerrep', function(source)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local curRep = Player.PlayerData.metadata["dealerrep"]
...
-- trade tobacco
RegisterServerEvent('legends-indiantrader:server:tradetobacco')
AddEventHandler('legends-indiantrader:server:tradetobacco', function(data) -- 1 / 5 / 10
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
...
-- remove item
RegisterServerEvent('legends-indiantobacco:server:removeitem')
AddEventHandler('legends-indiantobacco:server:removeitem', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
