...
function DestroyPlant()
    ...
    if not hasDone then
        ...
        local ped = PlayerPedId()
        isDoingAction = true
        TriggerServerEvent('legends-indiantobacco:server:plantHasBeenHarvested', plant.id)
        TaskStartScenarioInPlace(ped, `WORLD_HUMAN_CROUCH_INSPECT`, 0, true)
        Wait(5000)
        ClearPedTasks(ped)
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        TriggerServerEvent('legends-indiantobacco:server:destroyPlant', plant.id)
        isDoingAction = false
        canHarvest = true
    else
...
function HarvestPlant()
    ...
    if not hasDone then
        ...
        ClearPedTasks(ped)
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        TriggerServerEvent('legends-indiantobacco:server:harvestPlant', plant.id)
        isDoingAction = false
        canHarvest = true
    else
        RSGCore.Functions.Notify('error', 'error')
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) < 1.3 and not isDoingAction and not v.beingHarvested and not IsPedInAnyVehicle(PlayerPedId(), false) then
                if PlayerJob.name == 'police' then
                    ...
                    if v.growth < 100 then
                        ...
                        if IsControlJustPressed(0, RSGCore.Shared.Keybinds['G']) then
                            if v.id == plant.id then
                                TriggerEvent('legends-indiantobacco:client:waterPlant')
                            end
                        elseif IsControlJustPressed(0, RSGCore.Shared.Keybinds['J']) then
                            if v.id == plant.id then
                                TriggerEvent('legends-indiantobacco:client:feedPlant')
                            end
                        end
                    else
...
RegisterNetEvent('legends-indiantobacco:client:removePlantObject')
AddEventHandler('legends-indiantobacco:client:removePlantObject', function(plant)
...
-- water plants
RegisterNetEvent('legends-indiantobacco:client:waterPlant')
AddEventHandler('legends-indiantobacco:client:waterPlant', function()
    local entity = nil
    local plant = GetClosestPlant()
    local ped = PlayerPedId()
    isDoingAction = true
    ...
    if hasItem then
        ...
        ClearPedTasks(ped)
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        TriggerServerEvent('legends-indiantobacco:server:waterPlant', plant.id)
        isDoingAction = false
    else
...
-- feed plants
RegisterNetEvent('legends-indiantobacco:client:feedPlant')
AddEventHandler('legends-indiantobacco:client:feedPlant', function()
    local entity = nil
    local plant = GetClosestPlant()
    local ped = PlayerPedId()
    isDoingAction = true
    ...
    if hasItem then
        ...
        ClearPedTasks(ped)
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        TriggerServerEvent('legends-indiantobacco:server:feedPlant', plant.id)
        isDoingAction = false
    else
...
RegisterNetEvent('legends-indiantobacco:client:updatePlantData')
AddEventHandler('legends-indiantobacco:client:updatePlantData', function(data)
    Config.Plants = data
end)

RegisterNetEvent('legends-indiantobacco:client:plantNewSeed')
AddEventHandler('legends-indiantobacco:client:plantNewSeed', function(type)
    ...
    if CanPlantSeedHere(pos) and not IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        ClearPedTasks(ped)
        SetCurrentPedWeapon(ped, `WEAPON_UNARMED`, true)
        TriggerServerEvent('legends-indiantobacco:server:plantNewSeed', type, pos)
    else
...
RegisterNetEvent('legends-indiantobacco:client:SmokeIndian')
AddEventHandler('legends-indiantobacco:client:SmokeIndian', function()
    ...
    TriggerServerEvent('legends-indiantobacco:server:removeitem', 'indiancigar', 1)
    -- fill up cores
