...
Citizen.CreateThread(function()
    for butcher, v in pairs(Config.ButcherLocations) do
        local name = v.name
        exports['legends-core']:createPrompt(v.location, v.coords, RSGCore.Shared.Keybinds['J'],  Lang:t('menu.open') .. v.name, {
            type = 'client',
            event = 'legends-butcher:client:menu',
            args = { name },
        })
...
-- butcher menu
RegisterNetEvent('legends-butcher:client:menu', function(butchername)
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            icon = "fas fa-paw",
            params = {
                event = 'legends-butcher:client:sellanimal',
                isServer = false,
                args = {}
            }
        },
        {
            ...
            params = {
                event = 'legends-butcher:client:OpenButcherShop',
                isServer = false,
                args = {}
            }
        },
...
RegisterNetEvent('legends-butcher:client:sellanimal')
AddEventHandler('legends-butcher:client:sellanimal', function()
                ...
                RSGCore.Functions.Progressbar('sell-carcass',  Lang:t('progressbar.selling')..name..'..', Config.SellTime, false, true, {
                    ...
                    if deleted then
                        if quality == false then
                            TriggerServerEvent('legends-butcher:server:reward', rewardmoney, rewarditem, 'poor') -- poor quality reward
                        end
                        if quality == 0 then
                            TriggerServerEvent('legends-butcher:server:reward', rewardmoney, rewarditem, 'poor') -- poor quality reward
                        end
                        if quality == 1 then
                            TriggerServerEvent('legends-butcher:server:reward', rewardmoney, rewarditem, 'good') -- good quality reward
                        end
                        if quality == 2 then
                            TriggerServerEvent('legends-butcher:server:reward', rewardmoney, rewarditem, 'perfect') -- perfect quality reward
                        end
                        if quality == -1 then
                            TriggerServerEvent('legends-butcher:server:reward', rewardmoney, rewarditem, 'perfect') -- perfect quality reward
                        end
...
RegisterNetEvent('legends-butcher:client:OpenButcherShop')
AddEventHandler('legends-butcher:client:OpenButcherShop', function()
    local ShopItems = {}
