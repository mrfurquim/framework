                ...
                if IsControlJustReleased(0, RSGCore.Shared.Keybinds['J']) then
                    RSGCore.Functions.TriggerCallback('police:GetCops', function(result)
                        ...
                        if CurrentLawmen >= Config.MinimumLawmen then
                            ...
                            if hasItem then
                                TriggerServerEvent('legends-valentinebankheist:server:removeItem', 'lockpick', 1)
    ...
    while true do
        ...
        if object ~= 0 and robberystarted == true and dynamiteused == false then
            ...
            if #(pos - objectPos) < 3.0 then
                ...
                if IsControlJustReleased(0, RSGCore.Shared.Keybinds['J']) then
                    TriggerEvent('legends-valentinebankheist:client:boom')
                end
            end
        end
        if awayFromObject then
...
-- blow vault doors
RegisterNetEvent('legends-valentinebankheist:client:boom')
AddEventHandler('legends-valentinebankheist:client:boom', function()
    if robberystarted == true then
        local hasItem = RSGCore.Functions.HasItem('dynamite', 1)
        if hasItem then
            dynamiteused = true
            TriggerServerEvent('legends-valentinebankheist:server:removeItem', 'dynamite', 1)
            ...
            TriggerEvent('legends-valentinebankheist:client:policenpc')
...
Citizen.CreateThread(function()
    exports['legends-core']:createPrompt('valvault1', vector3(-308.26, 762.72, 118.7), 0xF3830D8E, 'Loot Vault', {
        type = 'client',
        event = 'legends-valentinebankheist:client:checkvault1',
        args = {},
    })
end)

-- loot vault1
RegisterNetEvent('legends-valentinebankheist:client:checkvault1', function()
    ...
    if robberystarted == true and vault1 == false then
            ...
            SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
            TriggerServerEvent('legends-valentinebankheist:server:reward')
            vault1 = true
    else
...
Citizen.CreateThread(function()
    exports['legends-core']:createPrompt('valvault2', vector3(-308.7, 765.22, 118.7), 0xF3830D8E, 'Loot Vault', {
        type = 'client',
        event = 'legends-valentinebankheist:client:checkvault2',
        args = {},
    })
end)

-- loot vault2
RegisterNetEvent('legends-valentinebankheist:client:checkvault2', function()
    local player = PlayerPedId()
    SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
...
-- start mission npcs
RegisterNetEvent('legends-valentinebankheist:client:policenpc')
AddEventHandler('legends-valentinebankheist:client:policenpc', function()
...
Citizen.CreateThread(function()
    exports['legends-core']:createPrompt('valpolicelock', vector3(-302.72, 772.64, 118.7), RSGCore.Shared.Keybinds['J'], 'Emergency Menu', {
        type = 'client',
        event = 'legends-valentinebankheist:client:bankmenu',
        args = {},
    })
end)

-- emergency menu
RegisterNetEvent('legends-valentinebankheist:client:bankmenu', function()
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            icon = "fas fa-lock",
            params = {
                event = 'legends-valentinebankheist:client:policelock',
                isServer = false,
            }
        },
        {
            ...
            params = {
                event = 'legends-valentinebankheist:client:policeunlock',
                isServer = false,
            }
        },
        {
...
RegisterNetEvent('legends-valentinebankheist:client:policelock', function()
...
RegisterNetEvent('legends-valentinebankheist:client:policeunlock', function()
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            -- lock doors
            for k,v in pairs(Config.VaultDoors) do
