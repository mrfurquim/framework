...
--[[ -- Only used by the following "legends-banking:initiateTransfer"
...
RegisterNetEvent('legends-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.legends-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.legends-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('legends-banking:openBankScreen', _src)
                    TriggerClientEvent('legends-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('legends-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
RSGCore.Functions.CreateCallback('legends-banking:getBankingInformation', function(source, cb)
...
RegisterNetEvent('legends-banking:createBankCard', function(pin)
    local src = source
    ...
    TriggerClientEvent('legends-banking:openBankScreen', src)
    TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('legends-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('legends-banking:openBankScreen', src)
            TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('legends-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('legends-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('legends-banking:openBankScreen', src)
            TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('legends-banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = RSGCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        MySQL.update('UPDATE bank_cards SET cardPin = ? WHERE record_id = ?', {
            ...
            if result == 1 then
                TriggerClientEvent('legends-banking:openBankScreen', src)
                TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.updated_pin'))
            else
                --TriggerClientEvent('RSGCore:Notify', src, 'Error updating pin', "error")
                TriggerClientEvent('Notification:Legends','Opss' , 'Erro ao atualizar PIN' , 'menu_textures', 'cross', 3000)
            end
        end)
    end
end)

RegisterNetEvent('legends-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('legends-banking:openBankScreen', src)
        TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('legends-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('legends-banking:openBankScreen', src)
        TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('legends-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = RSGCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('legends-banking:openBankScreen', src)
    TriggerClientEvent('legends-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('legends-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)
                    ...
                    if amount > 0 then
                        if xPlayer.Functions.RemoveMoney('cash', amount) then
                            if xReciv.Functions.AddMoney('cash', amount) then
                                ...
                                TriggerClientEvent('Notification:Legends','Sucesso'..id, 'Cash'..amount, 'menu_textures', 'cross', 3000)
                                --TriggerClientEvent('RSGCore:Notify', id, Lang:t('success.received_cash',{id = tostring(src), cash = tostring(amount)}), "success")
                                TriggerClientEvent('Notification:Legends','Oba..'..id , 'Cash Recebido'..amount , 'menu_textures', 'cross', 3000)
                                TriggerClientEvent("payanimation", src)
                            else
                                -- Return player cash
                                xPlayer.Functions.AddMoney('cash', amount)
                               -- TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.not_give'), "error")
                               TriggerClientEvent('Notification:Legends','Ops..' , 'Não enviado' , 'menu_textures', 'cross', 3000)
                            end
                        else
                            --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.not_enough'), "error")
                            TriggerClientEvent('Notification:Legends','Ops..' , 'Cash Insuficiente' , 'menu_textures', 'cross', 3000)
                        end
                    else
                        --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.invalid_amount'), "error")
                        TriggerClientEvent('Notification:Legends','Ops..' , 'Quantidade Inválida' , 'menu_textures', 'cross', 3000)
                    end
                else
                   --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.too_far_away'), "error")
                   TriggerClientEvent('Notification:Legends','Ops..' , 'Muito Distante' , 'menu_textures', 'cross', 3000)
                end
            else
                --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.dead'), "error")
                TriggerClientEvent('Notification:Legends','Ops..' , 'Pessoa Morta' , 'menu_textures', 'cross', 3000)
            end
        else
            --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.wrong_id'), "error")
            TriggerClientEvent('Notification:Legends','Ops..' , 'ID Errado' , 'menu_textures', 'cross', 3000)
        end
    else
        --TriggerClientEvent('RSGCore:Notify', src, Lang:t('error.givecash'), "error")
        TriggerClientEvent('Notification:Legends','Ops..' , 'Erro ao enviar' , 'menu_textures', 'cross', 3000)
    end
end)
