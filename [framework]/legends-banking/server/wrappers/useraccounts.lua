        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('legends-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = RSGCore.Functions.GetPlayer(self.source)
...
RegisterNetEvent('legends-banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('legends-banking:server:registerSavingsAccount', function(cid)
