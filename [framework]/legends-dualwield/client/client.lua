...
local function givePlayerWeapon(weaponName, attachPoint)
    ...
    Wait(1000)
    -- GIVE_WEAPON_TO_PED
    Citizen.InvokeNative("0x5E3BDDBCB83F3D84", PlayerPedId(), weaponHash, true, false, attachPoint, true, 0.0, 0.0, addReason, true, 0.0, false);
end
 
RegisterNetEvent('legends-dualwield:client:dualwield', function()
    local ped = PlayerPedId()
	local hasWeapon1 = RSGCore.Functions.HasItem('weapon_revolver_cattleman', 1)
	local hasWeapon2 = RSGCore.Functions.HasItem('weapon_revolver_lemat', 1)
	if hasWeapon1 and hasWeapon2 then
		addWardrobeInventoryItem("CLOTHING_ITEM_M_OFFHAND_000_TINT_004", 0xF20B6B4A);
		addWardrobeInventoryItem("UPGRADE_OFFHAND_HOLSTER", 0x39E57B01);
		givePlayerWeapon("WEAPON_REVOLVER_CATTLEMAN", 2);
		givePlayerWeapon("WEAPON_REVOLVER_LEMAT", 3);
	else
		RSGCore.Functions.Notify('Cattleman and Lemat revolver required!', 'error')
	end
end)

RegisterNetEvent('legends-dualwield:client:dualwield', function()
    local ped = PlayerPedId()
	local hasWeapon1 = RSGCore.Functions.HasItem('weapon_revolver_schofield', 1)
	local hasWeapon2 = RSGCore.Functions.HasItem('weapon_revolver_navy', 1)
	if hasWeapon1 and hasWeapon2 then
		addWardrobeInventoryItem("CLOTHING_ITEM_M_OFFHAND_000_TINT_004", 0xF20B6B4A);
		addWardrobeInventoryItem("UPGRADE_OFFHAND_HOLSTER", 0x39E57B01);
		givePlayerWeapon("WEAPON_REVOLVER_SCHOFIELD", 2);
		givePlayerWeapon("WEAPON_REVOLVER_NAVY", 3);
	else
		RSGCore.Functions.Notify('Schofield and NAVY revolver required!', 'error')
	end
end)
