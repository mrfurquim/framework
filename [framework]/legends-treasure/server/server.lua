...
RSGCore.Functions.CreateUseableItem("treasure1", function(source, item)
    ...
    TriggerClientEvent("legends-treasure:client:gototreasure", src, coords, item.name)
end)

-----------------------------------------------------------------------------------

RegisterNetEvent('legends-treasure:server:givereward', function()
    local src = source
...
-- remove item/amount
RegisterServerEvent('legends-treasure:server:removeitem')
AddEventHandler('legends-treasure:server:removeitem', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
