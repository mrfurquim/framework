...
RegisterNetEvent('legends-treasure:client:gototreasure')
AddEventHandler('legends-treasure:client:gototreasure', function(treasureCoords, item)
    StartGpsMultiRoute(GetHashKey("COLOR_RED"), true, true)
    AddPointToGpsMultiRoute(treasureCoords)
    SetGpsMultiRouteRender(true)
    TriggerServerEvent('legends-treasure:server:removeitem', item, 1)
            ...
            if weaponhash == -862059856 and cooldownSecondsRemaining == 0 and isonmount == false then
                ...
                if dist < 1 then
                    ...
                    if IsControlJustReleased(0, 0xCEFD9220) then -- [E]
                        ...
                        TriggerEvent('legends-treasure:clent:digging', v.name)
                        handleCooldown()
                    end
                end
            end
...
-- dig for treasure
RegisterNetEvent("legends-treasure:clent:digging")
AddEventHandler("legends-treasure:clent:digging", function(chest)
    local hasItem = RSGCore.Functions.HasItem('shovel', 1)
    if hasItem then
        local randomNumber = math.random(1,100)
        if randomNumber > 90 then
            TriggerServerEvent('legends-treasure:server:removeitem', 'shovel', 1)
            ...
            Wait(10000)
            TriggerServerEvent('legends-treasure:server:givereward', chest)
            active = false
        end
    else
