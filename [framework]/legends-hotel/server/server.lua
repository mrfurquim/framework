...
-- rent room
RegisterNetEvent('legends-hotel:server:RentRoom', function(data)
    local src = source
...
-- room check-in
RegisterNetEvent('legends-hotel:server:CheckIn', function(data)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local location = data.location
    ...
    if result ~= 0 then
        ...
        TriggerClientEvent('legends-hotel:client:gotoRoom', src, location)
    else
...
RSGCore.Functions.CreateCallback('legends-hotel:server:GetActiveRoom', function(source, cb)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('legends-hotel:server:addcredit', function(newcredit, removemoney, roomid)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local cashBalance = Player.PlayerData.money["cash"]
...
RegisterNetEvent('legends-hotel:server:setdefaultbucket', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    ...
    if result then
        for i = 1, #result do
            ...
            if row.credit >= Config.RentPerCycle then
                ...
                TriggerEvent('legends-log:server:CreateLog', 'hotel', 'Hotel Room Lost', 'red', row.fullname..' room '..row.roomid..' in '..row.location..' has been deleted')
            end
        end
    end
