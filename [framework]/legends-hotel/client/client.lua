...
Citizen.CreateThread(function()
    for hotel, v in pairs(Config.HotelLocations) do
        exports['legends-core']:createPrompt(v.prompt, v.coords, RSGCore.Shared.Keybinds['J'], Lang:t('menu.open') .. v.name, {
            type = 'client',
            event = 'legends-hotel:client:menu',
...
-- hotel menu
RegisterNetEvent('legends-hotel:client:menu', function(hotelname, hotellocation)
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = 'legends-hotel:server:CheckIn',
                isServer = true,
        ...
        },
        {
            ...
            icon = "fas fa-bed",
            params = {
                event = 'legends-hotel:server:RentRoom',
                isServer = true,
                args = { location = hotellocation }
            }
        },
...
RegisterNetEvent('legends-hotel:client:gotoRoom', function(location)
...
Citizen.CreateThread(function()
    for hotelexit, v in pairs(Config.HotelRoom) do
        exports['legends-core']:createPrompt(v.prompt, v.coords, RSGCore.Shared.Keybinds['J'], Lang:t('menu.room_menu'), {
            type = 'client',
            event = 'legends-hotel:client:roommenu',
            args = { v.location },
        })
    end
end)
...
-- room menu
RegisterNetEvent('legends-hotel:client:roommenu', function()
    RSGCore.Functions.TriggerCallback('legends-hotel:server:GetActiveRoom', function(result)
        local activeRoom = {
            {
        ...
        activeRoom[#activeRoom+1] = {
            ...
            params = {
                event = "legends-hotel:client:addcredit",
                isServer = false,
        ...
        activeRoom[#activeRoom+1] = {
            ...
            icon = "fas fa-box",
            params = {
                event = "legends-hotel:client:roomlocker",
                isServer = false,
        ...
        activeRoom[#activeRoom+1] = {
            ...
            params = {
                event = "legends-hotel:client:minibar",
                isServer = false,
        ...
        activeRoom[#activeRoom+1] = {
            ...
            params = {
                event = 'legends-hotel:client:leaveroom',
                isServer = false,
        ...
        exports['legends-menu']:openMenu(activeRoom)
    end)
end)

--------------------------------------------------------------------------------------------------

RegisterNetEvent('legends-hotel:client:addcredit', function(data)
    ...
    if dialog ~= nil then
        for k,v in pairs(dialog) do
            ...
            TriggerServerEvent('legends-hotel:server:addcredit', tonumber(newcredit), tonumber(dialog.addcredit), data.room)
        end
    end
end)
...
-- leave room
RegisterNetEvent('legends-hotel:client:leaveroom')
AddEventHandler('legends-hotel:client:leaveroom', function(data)
    if Config.Debug == true then
        print(data.exitroom)
    end
    ...
    if roomlocation == 'valentine' then
        DoScreenFadeOut(500)
        Wait(1000)
        TriggerServerEvent('legends-hotel:server:setdefaultbucket')
...
-- room storage locker
RegisterNetEvent('legends-hotel:client:roomlocker', function(data)
...
RegisterNetEvent('legends-hotel:client:minibar')
AddEventHandler('legends-hotel:client:minibar', function(data)
    local ShopItems = {}
