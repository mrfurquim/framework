...
function RSGCore.Functions.HasItem(items, amount)
    return exports['legends-inventory']:HasItem(items, amount)
end

-- Utility

function RSGCore.Functions.GetCurrentTemperature()
    local player = PlayerPedId()
    local coords = GetEntityCoords(player)
    local temperature = math.floor(GetTemperatureAtCoords(coords.x, coords.y, coords.z))
    return temperature
end
...
function RSGCore.Functions.NotifyLeft(title, subTitle, dict, icon, duration)
...
function RSGCore.Functions.GetCorpTemperature()
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)
    local temperaturaCorporea = RSGCore.Functions.GetCurrentTemperature()
    local ClothesCats = {
        0x9925C067, --hat
        0x2026C46D, --shirt
        0x1D4C528A, -- pants
    ...
    for k,v in pairs(ClothesCats) do
        local IsWearingClothes = Citizen.InvokeNative(0xFB4891BD7578CDC1 ,ped, v)
        if IsWearingClothes then 
            temperaturaCorporea = temperaturaCorporea + 1
        end
    end
