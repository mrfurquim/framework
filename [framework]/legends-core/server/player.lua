...
function RSGCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState("legends-inventory") ~= "missing"
            and exports["legends-inventory"]:LoadInventory(PlayerData.source, PlayerData.citizenid)
        or {}
...
function RSGCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    self.Functions.AddXp = function(skill, amount)
        ...
        if self.PlayerData.metadata['xp'][skill] and amount > 0 then
            ...
            self.Functions.UpdatePlayerData()
            TriggerEvent('legends-log:server:CreateLog', 'levels', 'AddXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** has received: '..amount..'xp in the skill: '..skill..'. Their current xp amount is: '..self.PlayerData.metadata['xp'][skill])
            return true
        end
        return false
    end
    ...
    self.Functions.RemoveXp = function(skill, amount)
        ...
        if self.PlayerData.metadata['xp'][skill] and amount > 0 then
            ...
            self.Functions.UpdateLevelData(skill)
            self.Functions.UpdatePlayerData()
            TriggerEvent('legends-log:server:CreateLog', 'levels', 'RemoveXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' (citizenid: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..')** was stripped of: '..amount..'xp in the skill: '..skill..'. Their current xp amount is: '..self.PlayerData.metadata['xp'][skill])
            return true
        end
        return false
    end
    ...
    self.Functions.AddLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdatePlayerData()
            TriggerEvent('legends-log:server:CreateLog', 'levels', 'AddXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** got '..level..' levels '..amount..' [current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    self.Functions.RemoveLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdateLevelData(level)
            self.Functions.UpdatePlayerData()
            TriggerEvent('legends-log:server:CreateLog', 'levels', 'RemoveXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** lost '..level..' levels '..amount..' current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if moneytype == "bank" then
                TriggerClientEvent("legends-phone:client:RemoveBankMoney", self.PlayerData.source, amount)
            end
            TriggerClientEvent(
    ...
    function self.Functions.GetCardSlot(cardNumber, cardType)
        ...
        local slots = exports["legends-inventory"]:GetSlotsByItem(self.PlayerData.items, item)
...
function RSGCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState("legends-inventory") ~= "missing" then
            exports["legends-inventory"]:SaveInventory(source)
        end
...
function RSGCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState("legends-inventory") ~= "missing" then
            exports["legends-inventory"]:SaveInventory(PlayerData, true)
        end
...
function RSGCore.Player.SaveInventory(source)
    ...
    exports["legends-inventory"]:SaveInventory(source, false)
end
...
function RSGCore.Player.GetTotalWeight(items)
    ...
    return exports["legends-inventory"]:GetTotalWeight(items)
end
...
function RSGCore.Player.GetSlotsByItem(items, itemName)
    ...
    return exports["legends-inventory"]:GetSlotsByItem(items, itemName)
end
...
function RSGCore.Player.GetFirstSlotByItem(items, itemName)
    ...
    return exports["legends-inventory"]:GetFirstSlotByItem(items, itemName)
end
...
function RSGCore.Player.CreateWalletId()
    ...
    local WalletId = nil
    while not UniqueFound do
        WalletId = "LEG-" .. math.random(11111111, 99999999)
