            ...
            if Player then
                ...
                if Player.PlayerData.job and payment > 0 and (RSGShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
                    if RSGCore.Config.Money.PayCheckSociety then
                        local account = exports['legends-management']:GetAccount(Player.PlayerData.job.name)
                        if account ~= 0 then -- Checks if player is employed by a society
                            if account < payment then -- Checks if company has enough money to pay society
                                ...
                                exports['legends-management']:RemoveMoney(Player.PlayerData.job.name, payment)
...
function RSGCore.Functions.UseItem(source, item)
    if GetResourceState('legends-inventory') == 'missing' then return end
    exports['legends-inventory']:UseItem(source, item)
end
...
function RSGCore.Functions.HasItem(source, items, amount)
    if GetResourceState('legends-inventory') == 'missing' then return end
    return exports['legends-inventory']:HasItem(source, items, amount)
end
...
function RSGCore.Functions.NotifyLeft(title, subTitle, dict, icon, duration, color)
    TriggerClientEvent('RSGCore.NotifyLeft', title, subTitle, dict, icon, duration, color)
end
function RSGCore.Functions.GetPlayer(playerId)
    playerId = tonumber(playerId)
    if RSGCore.Players then
        if RSGCore.Players[playerId] then
            return RSGCore.Players[playerId]
        else
            return nil
        end
    else
