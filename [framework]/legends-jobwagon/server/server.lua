...
-- setup wagon
RegisterServerEvent('legends-jobwagon:server:SetupWagon', function()
    local src = source
...
-- get active wagon
RSGCore.Functions.CreateCallback('legends-jobwagon:server:GetActiveWagon', function(source, cb)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    local job = Player.PlayerData.job.name
