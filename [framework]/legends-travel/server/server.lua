...
RegisterServerEvent('legends-travel:server:buyticket')
AddEventHandler('legends-travel:server:buyticket', function(price)
	local src = source
...
-- remove ticket
RegisterNetEvent('legends-travel:server:removeItem', function(item, amount)
	local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
