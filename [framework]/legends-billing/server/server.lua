...
RSGCore.Functions.CreateCallback('legends-billing:server:checkbills', function(source, cb, target)
...
RSGCore.Functions.CreateCallback('legends-billing:server:checkSentBills', function(source, cb, citizenid)
    local src = source
...
-- pay bills
RegisterNetEvent('legends-billing:server:paybills', function(data)
    ...
    if data.society == 'personal' then
        ...
        if PayingPlayer.PlayerData.money.cash >= data.amount then
            PayingPlayer.Functions.RemoveMoney("cash", data.amount, "pay-bill")
            exports['legends-management']:AddMoney(data.society, data.amount)
            exports.oxmysql:execute('DELETE FROM player_bills WHERE id = ?', {data.billid})
            TriggerClientEvent('RSGCore:Notify', src, 'Bill has been paid for '..data.amount..'$', 'success')
        else
            TriggerClientEvent('RSGCore:Notify', src, 'You not have enough money', 'error')
...
-- cancel bill
RegisterNetEvent('legends-billing:server:cancelbill', function(billid)
    if Config.Debug == true then
        print(billid)
    end
    local src = source
...
RegisterNetEvent('legends-billing:server:sendSocietyBill', function(playerid, amount, society)
    local src = source
...
RegisterNetEvent('legends-billing:server:sendPlayerBill', function(playerid, amount)
    local src = source
    local SendPlayer = RSGCore.Functions.GetPlayer(src)
    local SendName = (SendPlayer.PlayerData.charinfo.firstname..' '..SendPlayer.PlayerData.charinfo.lastname)
    local Player = RSGCore.Functions.GetPlayer(tonumber(playerid))
...
RSGCore.Commands.Add("billing", "Opens the Billing Menu", {}, false, function(source)
    local src = source
    TriggerClientEvent('legends-billing:client:billingMenu', src)
end)
