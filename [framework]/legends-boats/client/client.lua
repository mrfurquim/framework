                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, OpenReturn) then -- UiPromptHasStandardModeCompleted
                                ...
                                if currentLocation == boatHome then
                                    ...
                                    if TransferAllow then
                                        ...
                                        TriggerServerEvent("legends-boats:TransferBoat", OwnedData, currentLocation, driveTransfer)
                                        ReturnBoat(shopId)
...
function MainMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value == "own" then
            ...
            TriggerServerEvent('legends-boats:GetOwnedBoats', location, shopId)
        end
    end,
    function(data, menu)
        menu.close()
...
function BuyMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value then
            ...
            TriggerServerEvent('legends-boats:BuyBoat', buyData, location)
            menu.close()
            InMenu = false
...
RegisterNetEvent("legends-boats:OwnedBoatsMenu")
AddEventHandler("legends-boats:OwnedBoatsMenu", function(ownedBoats, shopId)
    MenuData.CloseAll()
    InMenu = true
    local elements = {}
...
function BoatMenu(shopId)
    ...
    function(data, menu)
        ...
        elseif data.current.value == "sell" then
            ...
            TriggerServerEvent('legends-boats:SellBoat', OwnedData, boatData)
            menu.close()
            InMenu = false
            ClearPedTasksImmediately(player)
            DisplayRadar(true)
    ...
    function(data, menu)
        ...
        if transferLocation then
            ...
            if transferLocation ~= location then
                TriggerServerEvent("legends-boats:TransferBoat", OwnedData, transferLocation, menuTransfer, boatData, shopName)
                menu.close()
                InMenu = false
                ClearPedTasksImmediately(PlayerPedId())
                DisplayRadar(true)
