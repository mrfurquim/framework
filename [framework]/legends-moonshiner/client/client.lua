...
-- setup moonshine
RegisterNetEvent('legends-moonshiner:client:moonshinekit')
AddEventHandler('legends-moonshiner:client:moonshinekit', function(itemName) 
    if moonshinekit ~= 0 then
        ...
        TriggerServerEvent('legends-moonshiner:server:startsmoke', pos)
        PlaySoundFrontend("SELECT", "RDRO_Character_Creator_Sounds", true, 0)
        moonshinekit = prop
    end
end, false)

RegisterNetEvent('legends-moonshiner:client:startsmoke')
AddEventHandler('legends-moonshiner:client:startsmoke', function(smokecoords)
        ...
        if moonshineObject ~= 0 and PlayerJob.name ~= Config.LawJobName then
            ...
            if #(pos - objectPos) < 3.0 then
                ...
                if IsControlJustReleased(0, RSGCore.Shared.Keybinds['J']) then
                    TriggerEvent('legends-moonshiner:client:craftmenu')
                end
            end
        else
            local objectPos = GetEntityCoords(moonshineObject)
...
-- moonshine menu
RegisterNetEvent('legends-moonshiner:client:craftmenu', function(data)
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = 'legends-moonshiner:client:moonshine',
                isServer = false,
            }
        },
        {
...
-- make moonshine
RegisterNetEvent("legends-moonshiner:client:moonshine")
AddEventHandler("legends-moonshiner:client:moonshine", function()
    if isBusy then
        return
    else
        ...
        if hasItems then
            ...
            ClearPedTasks(player)
            SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
            TriggerServerEvent('legends-moonshiner:server:givemoonshine', 1)
            PlaySoundFrontend("SELECT", "RDRO_Character_Creator_Sounds", true, 0)
...
Citizen.CreateThread(function()
    for k,v in pairs(Config.MoonshineVendor) do
        exports['legends-core']:createPrompt(v.uid, v.pos, RSGCore.Shared.Keybinds['J'], v.header, {
            type = 'client',
            event = 'legends-moonshiner:client:sellmenu',
            args = {v.uid}
        })  
    end
end)
...
RegisterNetEvent('legends-moonshiner:client:sellmenu') 
AddEventHandler('legends-moonshiner:client:sellmenu', function(menuid)
    local shoptable = {
        {
    ...
    for k,v in pairs(Config.MoonshineVendor) do
        if v.uid == menuid then
            for g,f in pairs(v.shopdata) do
                local lineintable = {
                    header = "<img src=nui://legends-inventory/html/images/"..f.image.." width=20px>"..f.title..Lang:t('menu.price')..f.price..')',
                    params = {
                        event = 'legends-moonshiner:client:sellcount',
                        args = {menuid, f}
                    }
                }
...
RegisterNetEvent('legends-moonshiner:client:sellcount') 
AddEventHandler('legends-moonshiner:client:sellcount', function(arguments)
    ...
    if inputdata ~= nil then
        for k,v in pairs(inputdata) do
            TriggerServerEvent('legends-moonshiner:server:sellitem', v,data)
        end
    end
end)
