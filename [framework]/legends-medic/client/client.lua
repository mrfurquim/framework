...
CreateThread(function()
    for medic, v in pairs(Config.MedicJobLocations) do
        exports['legends-core']:createPrompt(v.prompt, v.coords, RSGCore.Shared.Keybinds['J'], 'Open ' .. v.name, {
            type = 'client',
            event = 'legends-medic:client:mainmenu',
            args = { v.prompt },
        })
...
-- medic menu
RegisterNetEvent('legends-medic:client:mainmenu', function(location)
    ...
    if job == Config.JobRequired then
        ...
        exports['legends-menu']:openMenu({
            ...
            },
            {
                ...
                params = {
                    event = 'legends-medic:client:ToggleDuty',
                    isServer = false,
                }
            },
            {
                ...
                params = {
                    event = 'legends-medic:clent:OpenMedicSupplies',
                    isServer = false,
                }
            },
            {
                ...
                icon = "fas fa-box",
                params = {
                    event = 'legends-medic:clent:storage',
                    isServer = false,
                    args = {},
                }
            },
...
-- get medics on-duty
AddEventHandler('legends-medic:client:GetMedicsOnDuty', function()
    RSGCore.Functions.TriggerCallback('legends-medic:server:getmedics', function(mediccount)
    ...
    while true do
        ...
        if deathactive == true then
            ...
            if deathTimerStarted == true and deathSecondsRemaining == 0 and IsControlPressed(0, RSGCore.Shared.Keybinds['E']) then
                ...
                TriggerEvent('legends-medic:clent:revive')
                TriggerServerEvent('legends-medic:server:deathactions')
            end
            if deathactive and deathTimerStarted and deathSecondsRemaining < Config.DeathTimer and IsControlPressed(0, RSGCore.Shared.Keybinds['J']) and not medicCalled then
                ...
                TriggerServerEvent('legends-medic:server:medicAlert', text)
...
RegisterNetEvent('legends-medic:clent:revive', function()
    SetClosestRespawn()
...
-- admin revive
RegisterNetEvent('legends-medic:clent:adminRevive', function()
    local player = PlayerPedId()
    DoScreenFadeOut(500)
    Wait(1000)
...
-- player revive
RegisterNetEvent('legends-medic:clent:playerRevive', function()
    local player = PlayerPedId()
    DoScreenFadeOut(500)
    Wait(1000)
    local pos = GetEntityCoords(player, true)
...
function deathTimer()
    ...
    Citizen.CreateThread(function()
        while deathSecondsRemaining > 0 do
            ...
            TriggerEvent("legends-medic:client:GetMedicsOnDuty")
        end
    end)
end
    ...
    while true do
        if healthset == true then
            ...
            if lastHealth ~= health then
                TriggerServerEvent('legends-medic:server:SetHealth', health)
            end
            lastHealth = health
            Wait(1000)
        else
...
-- medic supplies
RegisterNetEvent('legends-medic:clent:OpenMedicSupplies')
AddEventHandler('legends-medic:clent:OpenMedicSupplies', function()
    local ShopItems = {}
...
-- medic storage
RegisterNetEvent('legends-medic:clent:storage', function()
    local job = RSGCore.Functions.GetPlayerData().job.name
...
RegisterNetEvent('legends-medic:client:KillPlayer', function()
    local ped = PlayerPedId()
...
AddEventHandler("onResourceStop", function(resourceName)
    ...
    for i = 1, #Config.MedicJobLocations do
        ...
        exports['legends-core']:deletePrompt(pos.prompt)
    end
