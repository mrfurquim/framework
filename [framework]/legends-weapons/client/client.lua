...
RegisterNetEvent('legends-weapons:client:AutoDualWield', function()
...
RegisterNetEvent('legends-weapons:client:UseWeapon', function(weaponData, shootbool)
    local ped = PlayerPedId()
    ...
    if not alreadyUsed and not UsedWeapons[tonumber(hash)] then
        ...
        if weaponName == 'weapon_bow' or weaponName == 'weapon_bow_improved' then
            if weaponData.info.ammo == nil or weaponData.info.ammo == 0 then
                ...
                if hasItem then
                    ...
                    TriggerServerEvent('legends-weapons:server:removeWeaponAmmoItem', 'ammo_arrow')
                else
        ...
        elseif string.find(weaponName, 'thrown') then
            ...
            TriggerServerEvent('legends-weapons:server:removeWeaponItem', weaponName, 1)
        else
        ...
        TriggerServerEvent('legends-weapons:server:LoadComponents', wepSerial, hash)
        ...
        TriggerServerEvent('legends-weapons:server:SaveAmmo', weaponData.info.serie, ammo, ammoclip)
...
-- Components Loader
RegisterNetEvent("legends-weapon:client:LoadComponents")
AddEventHandler("legends-weapon:client:LoadComponents", function(components, hash)
    Wait(500)
...
-- load ammo
RegisterNetEvent('legends-weapons:client:AddAmmo', function(ammotype, amount, ammo)
    local ped = PlayerPedId()
    ...
    if not valid_ammo then
       ...
       RSGCore.Functions.NotifyLeft(Lang:t('error.wrong_ammo_for_weapon'), '', 'menu_textures','cross', 3000)
        return
    end
    ...
    if total + math.floor(amount * 0.5) < max_ammo then
        if RSGCore.Shared.Weapons[weapon] then
            ...
            TriggerServerEvent('legends-weapons:server:removeWeaponAmmoItem', ammo_type)
        end
        return
    end

--    RSGCore.Functions.Notify(Lang:t('error.max_mmo_capacity'), 'error')
TriggerEvent('Notification:LegendsLeft',Lang:t('error.max_mmo_capacity'), '', 'menu_textures', 'cross', 3000)
end)
...
CreateThread(function()
    while true do
        ...
        if currentSerial ~= nil and heldWeapon ~= -1569615261 then
            TriggerServerEvent('legends-weapons:server:SaveAmmo', currentSerial, tonumber(getammo), tonumber(getammoclip))
        end
    end
end)
