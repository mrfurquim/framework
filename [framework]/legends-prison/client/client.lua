...
Citizen.CreateThread(function()
    for prison, v in pairs(Config.MenuLocations) do
        exports['legends-core']:createPrompt(v.prompt, v.coords, RSGCore.Shared.Keybinds['J'], 'Open ' .. v.name, {
            type = 'client',
            event = 'legends-prison:client:menu',
            args = {},
        })
...
-- prison menu
RegisterNetEvent('legends-prison:client:menu', function()
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            params = { 
                event = 'legends-prison:client:shop',
                isServer = false,
                args = {}
            }
        },
        {
            ...
            params = { 
                event = 'legends-prison:client:telegrammenu',
                isServer = false,
                args = {}
            }
        },
...
-- telegram menu
RegisterNetEvent('legends-prison:client:telegrammenu', function(data)
    exports['legends-menu']:openMenu({
        {
        ...
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:writemessage',
                isServer = false,
                args = {}
            }
        },
...
-- prison shop
RegisterNetEvent('legends-prison:client:shop')
AddEventHandler('legends-prison:client:shop', function()
    local ShopItems = {}
...
RegisterNetEvent('RSGCore:Client:OnPlayerLoaded', function()
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata["injail"] > 0 then
            TriggerEvent("legends-prison:client:Enter", PlayerData.metadata["injail"])
        end
    end)
end)
...
RegisterNetEvent("legends-prison:client:prisonclothes") -- prison outfit event
AddEventHandler("legends-prison:client:prisonclothes", function()
    local ped = PlayerPedId()
        ...
        while jailtimeMinsRemaining > 0 do
            ...
            if jailtimeMinsRemaining > 0 then
                if jailtimeMinsRemaining > 1 then
                    exports['legends-core']:DrawText('Freedom in '..jailtimeMinsRemaining..' mins!', 'left')
                    TriggerServerEvent('legends-prison:server:updateSentance', jailtimeMinsRemaining)
                else
                    exports['legends-core']:DrawText('Getting ready for release!', 'left')
                    TriggerServerEvent('legends-prison:server:updateSentance', jailtimeMinsRemaining)
                end
            else
                exports['legends-core']:HideText()
                TriggerEvent('legends-prison:client:freedom')
            end
        end
    end)
end
...
-- released from jail
RegisterNetEvent('legends-prison:client:freedom', function()
    TriggerServerEvent('legends-prison:server:FreePlayer')
    TriggerServerEvent('legends-prison:server:GiveJailItems')
    Wait(500)
    DoScreenFadeOut(1000)
    Wait(3000)
