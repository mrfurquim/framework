...
RegisterNetEvent('legends-prison:server:updateSentance', function(updatedTime)
    local src = source
...
-- remove player job
RegisterNetEvent('legends-prison:server:RemovePlayerJob', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
...
-- auto free player
RegisterNetEvent('legends-prison:server:FreePlayer', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('legends-prison:server:SaveJailItems', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('legends-prison:server:GiveJailItems', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    Wait(1000)
...
-- set criminal record
RegisterNetEvent('legends-prison:server:CheckRecordStatus', function()
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
