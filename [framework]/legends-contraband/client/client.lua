...
RegisterCommand('sellcontraband', function(source)
    TriggerEvent('legends-contraband:client:contrabandselling')
end)

RegisterNetEvent('legends-contraband:client:contrabandselling', function()
    RSGCore.Functions.TriggerCallback('police:GetCops', function(lawmen)
        ...
        if CurrentLawmen >= Config.MinimumLawmen then
            RSGCore.Functions.TriggerCallback('legends-contraband:server:contrabandselling:getAvailableContraband', function(result)
                if result ~= nil then
...
RegisterNetEvent('legends-contraband:client:refreshAvailableContraband', function(items)
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            pedDist = #(coords - pedCoords)
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('legends-contraband:server:robContraband', availableContraband[contrabandType].item, contrabandAmount)
                ...
                if pedDist < 1.5 and contrabandselling then
                    ...
                    if IsControlJustPressed(0, 0x5415BE48) then
                        ...
                        TriggerServerEvent('legends-contraband:server:sellContraband', availableContraband[contrabandType].item, contrabandAmount, randomPrice)
                        hasTarget = false
                        -- animation here
