...
RSGCore.Functions.CreateCallback('legends-contraband:server:contrabandselling:getAvailableContraband', function(source, cb)
...
RegisterNetEvent('legends-contraband:server:sellContraband', function(item, amount, price)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if Player then
        ...
        if hasItem.amount >= amount then
            ...
            TriggerClientEvent('legends-contraband:client:refreshAvailableContraband', src, AvailableContraband)
        else
            TriggerClientEvent('legends-contraband:client:contrabandselling', src)
        end
    end
end)

RegisterNetEvent('legends-contraband:server:robContraband', function(item, amount)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if Player then
        local AvailableContraband = {}
