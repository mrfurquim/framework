        ...
        if WeaponData ~= nil and WeaponData["name"] ~= "weapon_unarmed" then
            ...
            if weaponGroup ~= `group_thrown` then
                ...
                if not hasItem then
                    ...
                    TriggerServerEvent("legends-log:server:CreateLog", "anticheat",  Lang:t('log.weapon_removed'), "orange", '** @staff ** '..fullname..' had a weapon on them that they did not have in his inventory : anticheat has removed the weapon')
                end
            end
        end
    end
