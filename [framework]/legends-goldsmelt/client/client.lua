...
-- setup gold smelt
RegisterNetEvent('legends-goldsmelt:client:setupgoldsmelt')
AddEventHandler('legends-goldsmelt:client:setupgoldsmelt', function()
			...
			if smeltObject ~= 0 then
				...
				if #(pos - objectPos) < 3.0 then
					...
					if IsControlJustReleased(0, RSGCore.Shared.Keybinds['J']) then
						TriggerEvent('legends-goldsmelt:client:smeltmenu')
					end
				end
			end
		end
...
-- smelt menu
RegisterNetEvent('legends-goldsmelt:client:smeltmenu', function()
    smeltMenu = {}
    smeltMenu = {
        {
    ...
    for k, v in pairs(Config.SmeltOptions) do
        ...
        smeltMenu[#smeltMenu + 1] = {
            ...
            txt = text,
            params = {
                event = 'legends-goldsmelt:client:checkinggolditems',
                args = {
					name = v.name,
                    item = k,
    ...
    exports['legends-menu']:openMenu(smeltMenu)
end)

------------------------------------------------------------------------------------------------------

-- check player has the items
RegisterNetEvent('legends-goldsmelt:client:checkinggolditems', function(data)
	RSGCore.Functions.TriggerCallback('legends-goldsmelt:server:checkinggolditems', function(hasRequired)
    if (hasRequired) then
		if Config.Debug == true then
			print("passed")
		end
		TriggerEvent('legends-goldsmelt:client:dosmelt', data.name, data.item, tonumber(data.smelttime), data.receive)
	else
		if Config.Debug == true then
			print("failed")
		end
...
-- do smelting
RegisterNetEvent('legends-goldsmelt:client:dosmelt', function(name, item, smelttime, receive)
	...
	RSGCore.Functions.Progressbar('smelt-gold', Lang:t('progressbar.smelting_a')..name, smelttime, false, true, {
		...
		TriggerServerEvent('legends-goldsmelt:server:finishsmelting', smeltitems, receive)
	end)
end)
