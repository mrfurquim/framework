...
RSGCore.Functions.CreateUseableItem("goldsmelt", function(source, item)
    local src = source
	TriggerClientEvent('legends-goldsmelt:client:setupgoldsmelt', src, item.name)
end)

-- check player has items
RSGCore.Functions.CreateCallback('legends-goldsmelt:server:checkinggolditems', function(source, cb, smeltitems)
    local src = source
...
-- finish smelting
RegisterServerEvent('legends-goldsmelt:server:finishsmelting')
AddEventHandler('legends-goldsmelt:server:finishsmelting', function(smeltitems, receive)
	local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
	-- remove smeltitems
	for k, v in pairs(smeltitems) do
