...
local function CreateItemDrop(index)
	...
	if Config.UseTarget then
		exports['legends-target']:AddTargetEntity(dropItem, {
			options = {
				{
...
RegisterNetEvent("legends-inventory:client:closeinv", function()
    closeInventory()
end)
...
RegisterNetEvent('legends-inventory:client:giveAnim', function()
...
CreateThread(function()
    while true do
        ...
        DisableControlAction(0, RSGCore.Shared.Keybinds['6'])
        ...
        if IsDisabledControlPressed(0, RSGCore.Shared.Keybinds['6']) and IsInputDisabled(0) then -- 5 slot
			if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
				TriggerServerEvent("inventory:server:UseItemSlot", 41)
			end
        end

        if IsDisabledControlJustPressed(0, RSGCore.Shared.Keybinds['F1']) and IsInputDisabled(0) then -- z  Hotbar
            isHotbar = not isHotbar
            ToggleHotbar(isHotbar)
        end
    end
