...
$(document).on("dblclick", ".item-slot", function(e) {
    ...
    if (ItemData) {
        Inventory.Close();
        $.post(
            "https://legends-inventory/UseItem",
            JSON.stringify({
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://legends-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://legends-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post(
            "https://legends-inventory/Notify",
            JSON.stringify({
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        ...
        } else if (itemData.name == "moneyclip") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p><b>Cash:</b> $" + itemData.info.money + "</p>");
        } else {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
...
function handleDragDrop() {
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post(
                "https://legends-inventory/DropItem",
                JSON.stringify({
                    inventory: fromInventory,
                    item: fromData,
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://legends-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://legends-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://legends-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://legends-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://legends-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://legends-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://legends-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://legends-inventory/CloseInventory", JSON.stringify({}));
...
$(document).on("click", "#rob-money", function(e) {
    ...
    $.post(
        "https://legends-inventory/RobMoney",
        JSON.stringify({
            TargetId: TargetId,
        })
    );
...
$("#item-give").droppable({
    ...
    drop: function(event, ui) {
        ...
        $.post(
            "https://legends-inventory/GiveItem",
            JSON.stringify({
                inventory: fromInventory,
                item: fromData,
                amount: parseInt(amount),
