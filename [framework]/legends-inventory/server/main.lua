...
local function AddItem(source, item, amount, slot, info)
	...
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
		...
		elseif itemInfo['unique'] or (not slot or slot == nil) or itemInfo['type'] == 'weapon' then
			for i = 1, Config.MaxInventorySlots, 1 do
				if Player.PlayerData.items[i] == nil then
					...
					TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
			end
		...
		elseif Player.PlayerData.items[slot].amount == amount then
			...
			TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				if not Player.Offline then
					TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
				end
			...
			elseif Player.PlayerData.items[_slot].amount == amountToRemove then
				...
				TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
...
local function ClearInventory(source, filterItems)
	...
	TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** inventory cleared')
end
...
local function SetInventory(source, items)
	...
	TriggerEvent('legends-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerEvent("legends-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			TriggerClientEvent("legends-inventory:client:closeinv", id)
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if itemData.type == "weapon" then
		TriggerClientEvent("legends-weapons:client:UseWeapon", src, itemData, itemData.info.quality and itemData.info.quality > 0)
			...
			elseif RSGCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("legends-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
					local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("legends-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif RSGCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("legends-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
					local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("legends-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
				...
				if toInventory == nil or toInventory == 0 then
					...
					if toItemData then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("legends-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
						local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("legends-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = RSGCore.Shared.Items[fromItemData.name:lower()]
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("legends-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("legends-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
					TriggerEvent("legends-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif RSGCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		if RSGCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if RSGCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('legends-drugs:client:updateDealerItems', src, itemData, 1)
					RSGCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("legends-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('legends-drugs:client:updateDealerItems', src, itemData, fromAmount)
					RSGCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("legends-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
		...
		elseif RSGCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
			if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                itemData.info.quality = itemData.info.quality or 100
				AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
				TriggerClientEvent('legends-shops:client:UpdateShop', src, RSGCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
				RSGCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
				TriggerEvent("legends-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("legends-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("legends-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
					TriggerEvent("legends-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				toInventory = tonumber(toInventory)
...
RegisterNetEvent('legends-inventory:server:SaveStashItems', function(stashId, items)
    MySQL.insert('INSERT INTO stashitems (stash, items) VALUES (:stash, :items) ON DUPLICATE KEY UPDATE items = :items', {
        ['stash'] = stashId,
        ['items'] = json.encode(items)
    })
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				TriggerClientEvent('legends-inventory:client:giveAnim', src)
				TriggerClientEvent('legends-inventory:client:giveAnim', target)
			else
...
RSGCore.Functions.CreateCallback('legends-inventory:server:GetStashItems', function(_, cb, stashId)
	...
	if Player then
			if itemData then
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["legends-methlab"]:GenerateRandomLab()
...
RSGCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    ...
    if Player then
        ...
        RSGCore.Functions.Notify(source, "Player not online", 'error')
    end
end, 'admin')
