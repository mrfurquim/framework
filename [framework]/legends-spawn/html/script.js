...
$(document).on('click', '.location', function(evt) {
    ...
    if (type !== "lab") {
        ...
        $.post('https://legends-spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '#submit-spawn', function(evt) {
    ...
    if (spawnType !== "appartment") {
        $.post('https://legends-spawn/spawnplayer', JSON.stringify({
            spawnloc: location,
            typeLoc: spawnType
        }));
    } else {
        $.post('https://legends-spawn/chooseAppa', JSON.stringify({
            appType: location,
        }));
    }
});
...
function setupLocations(locations, myHouses) {
    ...
    $(parent).html("");
    $(parent).append('<div class="loclabel" id="location" data-location="null" data-type="lab" data-label="Onde Deseja Iniciar?"><p><span id="null">Onde Deseja Iniciar ?</span></p></div>')

    setTimeout(function() {
        $(parent).append('<div class="location" id="location" data-location="current" data-type="current" data-label="Ultima Localização"><p><span id="current-location">Ultima Localização</span></p></div>');
