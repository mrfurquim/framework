...
RegisterNetEvent('legends-spawn:client:openUI', function(value)
...
RegisterNUICallback('spawnplayer', function(data)
    ...
    elseif type == "house" then
        ...
        DoScreenFadeOut(500)
        Wait(2000)
        TriggerEvent('legends-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('RSGCore:Server:OnPlayerLoaded')
        TriggerEvent('RSGCore:Client:OnPlayerLoaded')
        TriggerServerEvent('legends-houses:server:SetInsideMeta', 0, false)
        TriggerServerEvent('legends-apartments:server:SetInsideMeta', 0, 0, false)
        FreezeEntityPosition(ped, false)
        FreezeEntityPosition(PlayerPedId(), false)
        RenderScriptCams(false, true, 500, true, true)
        SetCamActive(cam, false)
...
RegisterNetEvent('legends-spawn:client:SpawnOnLastLocationOnly', function()
    local ped = PlayerPedId()
    local PlayerData = RSGCore.Functions.GetPlayerData()
    local isJailed = PlayerData.metadata["injail"]
...
RegisterNetEvent('legends-spawn:client:setupSpawnUI', function(cData, new)
    if not new and RSG.SpawnOnLastLocationOnly then
        TriggerEvent('legends-spawn:client:SpawnOnLastLocationOnly')
        return
    end

    if not RSG.EnableApartments then
        TriggerEvent('legends-spawn:client:setupSpawns', cData, new, nil)
        TriggerEvent('legends-spawn:client:openUI', true)
        return
    end

    RSGCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result == nil then
            TriggerEvent('legends-spawn:client:setupSpawns', cData, true, Apartments.Locations)
            TriggerEvent('legends-spawn:client:openUI', true)
            return
        end

        TriggerEvent('legends-spawn:client:setupSpawns', cData, false, nil)
        TriggerEvent('legends-spawn:client:openUI', true)
        TriggerEvent("apartments:client:SetHomeBlip", result.type)
    end, cData.citizenid)
end)

RegisterNetEvent('legends-spawn:client:setupSpawns', function(cData, new, apps)
    newPlayer = new
    if not new then
        if RSG.EnableHouses then
            RSGCore.Functions.TriggerCallback('legends-spawn:server:getOwnedHouses', function(houses)
                local myHouses = {}
                if houses ~= nil then
                    for i = 1, (#houses), 1 do
