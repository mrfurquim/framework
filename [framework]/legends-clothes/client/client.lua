...
function OpenClothingMenu()
    ...
    table.insert(elements, {label = Config.Label["save"] or "Salvar", value = "save", desc = ""})
    MenuData.Open('default', GetCurrentResourceName(), 'clothing_store_menu',
            {title = 'Armário', subtext = 'Guarda-Roupas', align = 'top-left', elements = elements}, function(data, menu)
        if data.current.value ~= "save" then
            ...
            saveOutfit = true
            TriggerServerEvent("legends-clothes:Save", ClothesCache, output, CurrentPrice)
            OldClothesCache = {}
        end
...
RegisterNetEvent('legends-clothes:OpenClothingMenu')
AddEventHandler('legends-clothes:OpenClothingMenu', function(ClothesComponents)
...
function Change(id, category, change_type)
    if id < 1 then
        ...
        if category == "pants" or category == "boots" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['legends-appearance']:GetBodyCurrentComponentHash("BODIES_LOWER"), false, true, true)
        end
        if category == "shirts_full" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['legends-appearance']:GetBodyCurrentComponentHash("BODIES_UPPER"), false, true, true)
        end
    else
        if IsPedMale(PlayerPedId()) then
...
RegisterNetEvent('legends-clothes:ApplyClothes')
AddEventHandler('legends-clothes:ApplyClothes', function(ClothesComponents, Target)
    Citizen.CreateThread(function()
...
RegisterNetEvent('legends-clothes:OpenOutfits')
AddEventHandler('legends-clothes:OpenOutfits', function()
    TriggerServerEvent('legends-clothes:getOutfits')
end)

RegisterNetEvent('legends-clothes:putInTable')
AddEventHandler('legends-clothes:putInTable', function(outfit)
    Outfits_tab = {}
...
function OutfitsManage(outfit)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'outfits_menu_manage',
        ...
        menu.close()
        TriggerServerEvent('legends-clothes:' .. data.current.value, outfit)
    end, function(data, menu)
        Outfits()
    end)
end
