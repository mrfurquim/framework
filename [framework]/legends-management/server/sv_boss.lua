...
function ExploitBan(id, reason)
	...
	TriggerEvent('legends-log:server:CreateLog', 'bans', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(id), 'legends-management', reason), true)
...
RegisterNetEvent("legends-bossmenu:server:withdrawMoney", function(amount)
	local src = source
	...
	if RemoveMoney(job, amount) then
		...
		TriggerEvent('legends-log:server:CreateLog', 'bossmenu', 'Withdraw Money', "blue", Player.PlayerData.name.. "Withdrawal $" .. amount .. ' (' .. job .. ')', false)
	...
	TriggerClientEvent('legends-bossmenu:client:OpenMenu', src)
end)

RegisterNetEvent("legends-bossmenu:server:depositMoney", function(amount)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		local job = Player.PlayerData.job.name
		AddMoney(job, amount)
		TriggerEvent('legends-log:server:CreateLog', 'bossmenu', 'Deposit Money', "blue", Player.PlayerData.name.. "Deposit $" .. amount .. ' (' .. job .. ')', false)
...
RSGCore.Functions.CreateCallback('legends-bossmenu:server:GetAccount', function(_, cb, jobname)
	local result = GetAccount(jobname)
	cb(result)
end)

-- Get Employees
RSGCore.Functions.CreateCallback('legends-bossmenu:server:GetEmployees', function(source, cb, jobname)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
...
-- Grade Change
RegisterNetEvent('legends-bossmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
...
-- Fire Employee
RegisterNetEvent('legends-bossmenu:server:FireEmployee', function(target)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetJob("unemployed", '0') then
				TriggerEvent("legends-log:server:CreateLog", "bossmenu", "Job Fire", "red", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.job.name .. ")", false)
...
-- Recruit Player
RegisterNetEvent('legends-bossmenu:server:HireEmployee', function(recruit)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		...
		TriggerEvent('legends-log:server:CreateLog', 'bossmenu', 'Recruit', "lightgreen", (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. " successfully recruited " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. ' (' .. Player.PlayerData.job.name .. ')', false)
	end
	TriggerClientEvent('legends-bossmenu:client:OpenMenu', src)
end)

-- Get closest player sv
RSGCore.Functions.CreateCallback('legends-bossmenu:getplayers', function(source, cb)
	local src = source
	local players = {}
