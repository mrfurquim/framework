...
RegisterNetEvent("legends-gangmenu:server:withdrawMoney", function(amount)
	local src = source
	...
	if RemoveGangMoney(gang, amount) then
		...
		TriggerEvent('legends-log:server:CreateLog', 'gangmenu', 'Withdraw Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully withdrew $' .. amount .. ' (' .. gang .. ')', false)
	...
	TriggerClientEvent('legends-gangmenu:client:OpenMenu', src)
end)

RegisterNetEvent("legends-gangmenu:server:depositMoney", function(amount)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerEvent('legends-log:server:CreateLog', 'gangmenu', 'Deposit Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully deposited $' .. amount .. ' (' .. gang .. ')', false)
...
RSGCore.Functions.CreateCallback('legends-gangmenu:server:GetAccount', function(_, cb, GangName)
	local gangmoney = GetGangAccount(GangName)
	cb(gangmoney)
end)

-- Get Employees
RSGCore.Functions.CreateCallback('legends-gangmenu:server:GetEmployees', function(source, cb, gangname)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
...
-- Grade Change
RegisterNetEvent('legends-gangmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
...
-- Fire Member
RegisterNetEvent('legends-gangmenu:server:FireMember', function(target)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetGang("none", '0') then
				TriggerEvent("legends-log:server:CreateLog", "gangmenu", "Gang Fire", "orange", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.gang.name .. ")", false)
...
-- Recruit Player
RegisterNetEvent('legends-gangmenu:server:HireMember', function(recruit)
	local src = source
	local Player = RSGCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		...
		TriggerEvent('legends-log:server:CreateLog', 'gangmenu', 'Recruit', 'yellow', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
	end
	TriggerClientEvent('legends-gangmenu:client:OpenMenu', src)
end)

-- Get closest player sv
RSGCore.Functions.CreateCallback('legends-gangmenu:getplayers', function(source, cb)
	local src = source
	local players = {}
