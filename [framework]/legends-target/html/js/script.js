...
function FoundTarget(item) {
    ...
    $("#target-eye").css({"--fa-primary-color": "white", "--fa-secondary-color": "white", "--fa-secondary-opacity": "1.0"});
}
...
function LeftTarget() {
    ...
    $("#target-eye").attr("class", "fa-duotone fa-eye-slash");
...
function ValidTarget(item) {
    ...
    $.each(item.data, function(index, item) {
        ...
        $("#target-" + index).hover((e) => {
            $("#target-" + index).css("color", e.type === "mouseenter" ? "green" : "white")
        })
