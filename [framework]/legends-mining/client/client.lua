...
Citizen.CreateThread(function()
    for mining, v in pairs(Config.MiningLocations) do
        exports['legends-core']:createPrompt(v.location, v.coords, RSGCore.Shared.Keybinds['J'], Lang:t('menu.start') .. v.name, {
            type = 'client',
            event = 'legends-mining:client:StartMining',
        })
...
-- start mining
RegisterNetEvent('legends-mining:client:StartMining')
AddEventHandler('legends-mining:client:StartMining', function()
    ...
    if miningstarted == false then
        if hasItem then
            ...
            if randomNumber > 90 then -- 10% chance of pickace breaking
                TriggerServerEvent('legends-mining:server:removeitem', 'pickaxe')
            else
                ...
                TriggerEvent('legends-mining:client:MiningAnimation')
                ...
                RSGCore.Functions.Progressbar("mining", "Mining...", 18000, false, true,
                {
                    ...
                    TriggerServerEvent('legends-mining:server:giveMiningReward')
...
AddEventHandler('legends-mining:client:MiningAnimation', function()
