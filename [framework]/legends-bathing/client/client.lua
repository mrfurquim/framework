...
RegisterNetEvent('legends-bathing:client:StartBath')
AddEventHandler('legends-bathing:client:StartBath', function(town)
    inbath = true 
    if Config.BathingZones[town] then
        ...
        TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { rag, "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@RAG`, `DEFAULT`, "BATHING" })    
...
ExitBathing = function(animscene, town, cam)
    ...
    TriggerServerEvent("legends-bathing:server:setBathAsFree", town)
...
RegisterNetEvent('legends-bathing:client:StartDeluxeBath')
AddEventHandler('legends-bathing:client:StartDeluxeBath', function(animscene, town, cam)
    if not IsPedMale(PlayerPedId()) then
        ...
        TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@MAID`, `DEFAULT`, "BATHING" })    
...
RegisterNetEvent('legends-bathing:client:HideDeluxePrompt')
AddEventHandler('legends-bathing:client:HideDeluxePrompt', function()
...
ExitPremiumBath = function(animscene, town, cam, disableScrub)
    ...
    TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
    TriggerEvent("legends-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@REGULAR@MAID`, `DEFAULT`, "BATHING" })    
...
UndressCharacter = function()
    ...
    EquippedWeapons = exports['legends-weapons']:EquippedWeapons()

    for i = 1, #EquippedWeapons do
        RemoveWeaponFromPed(ped, EquippedWeapons[i])
    end
    TriggerEvent('legends-wardrobe:client:removeAllClothing')
end

DressCharacter = function()
    TriggerServerEvent('legends-appearance:LoadSkin')
end
...
Action = function(name, p1, p2, p3)
    ...
    if (name == "START_BATHING") then
        TriggerServerEvent("legends-bathing:server:canEnterBath", p1)
    elseif (name == "REQUEST_DELUXE_BATHING") then
        TriggerServerEvent("legends-bathing:server:canEnterDeluxeBath", p1 , p2 , p3)
