...
RegisterNetEvent('legends-fishing:client:usebait')
AddEventHandler('legends-fishing:client:usebait', function(UsableBait)
Citizen.CreateThread(function()
        ...
        TriggerServerEvent('legends-fishing:server:removeBaitItem', currentLure)
        while fishing do
            Wait(0)
                ...
                if FISHING_GET_MINIGAME_STATE() == 7 then
                    ...
                    if GetGameTimer() >= nextAttTime then
                        ...
                        if probabilidadePuxar > 0.8 or probabilidadePuxar < 0.2 then -- soltar linha
                            ...
                            local r = exports["legends-fishing"]:VERTICAL_PROBE(x, y,  z, 1)
                            local valid, height = r[1], r[2]
                                                        
                        -- import from ptfx on legends-fishing c# version
                ...
                if FISHING_GET_MINIGAME_STATE() == 12 then
                    if IsControlJustPressed(0, GetHashKey("INPUT_ATTACK")) then
                        if fishing then
                            ...
                            TriggerServerEvent("legends-fishing:FishToInventory", fishModel, fishWeight)
...
function GET_TASK_FISHING_DATA()
    local r = exports["legends-fishing"]:GET_TASK_FISHING_DATA_EXTRA()
    hasMinigameOn = r[1]
    local outAsInt = r[2]
...
function SET_TASK_FISHING_DATA()
    if fishing_minigame_struct.f_0 ~= nil then
        exports["legends-fishing"]:SET_TASK_FISHING_DATA_EXTRA(fishing_minigame_struct)
    end
end
