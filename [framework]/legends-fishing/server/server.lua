...
RSGCore.Functions.CreateUseableItem("p_baitbread01x", function(source, item)
    ...
    TriggerClientEvent("legends-fishing:client:usebait", source, item.name)
end)
...
RegisterServerEvent('legends-fishing:server:removeBaitItem')
AddEventHandler('legends-fishing:server:removeBaitItem', function(item)
    local src = source
...
RegisterServerEvent('legends-fishing:FishToInventory')
AddEventHandler("legends-fishing:FishToInventory", function(fishModel, weight)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    ...
    TriggerEvent('legends-log:server:CreateLog', 'fishing', 'Fish Caught', 'green', firstname..' '..lastname..' caught a '..fish_weight..'KG '..fish_name)
end)
