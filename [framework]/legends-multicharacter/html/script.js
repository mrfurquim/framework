    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                var originalText = "Recebendo dados dos Personagens";
                ...
                var DotsInterval = setInterval(function() {
                    ...
                    if (loadingProgress == 3) {
                        originalText = "Validando dados dos Personagens"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 4) {
                        originalText = "Recebendo personagens"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 6) {
                        originalText = "Validando Personagens"
                        $("#loading-text").html(originalText);
                    }
                    if(loadingDots == 4) {
                        $("#loading-text").html(originalText);
                ...
                setTimeout(function(){
					setCharactersList()
                    $.post('https://legends-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('https://legends-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://legends-multicharacter/closeUI');
    $.post('https://legends-multicharacter/disconnectButton');
});

function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">O Slot de personagen ainda não está em uso.<br><br>Este Personagem não tem informações ainda.</span>');
    } else {
        var gender = "Homem"
        if (cData.charinfo.gender == 1) { gender = "Mulher" }
        ...
        '<div class="character-info-box"><span id="info-label">Data de Aniversário: </span><span class="char-info-js">'+cData.charinfo.birthdate+'</span></div>' +
        ...
        '<div class="character-info-box"><span id="info-label">Dinheiro: </span><span class="char-info-js">&#36; '+cData.money.cash+'</span></div>' +
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://legends-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function (e) {
    ...
    if(regTest.test(firstname) || regTest.test(lastname)){
        console.log("ERRO: VOCÊ USOU UM TERMO VULGAR, CORRIJA E TENTE NOVAMENTE")
        return false;
    }

    $.post('https://legends-multicharacter/createNewCharacter', JSON.stringify({
        firstname: firstname,
        lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://legends-multicharacter/removeCharacter', JSON.stringify({
...
function setCharactersList() {
    var htmlResult = '<div class="character-list-header"><p>Meus Personagens</p></div>'
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">Espaço Vazio<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Selecione um Personagem</p></div><div class="character-btn" id="delete"><p id="delete-text">Selecione um Personagem</p></div>'
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://legends-multicharacter/selectCharacter', JSON.stringify({
