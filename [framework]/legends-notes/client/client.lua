...
RegisterNetEvent('RSGCore:client:OnPlayerLoaded', function()
    TriggerEvent("legends-notes:client:SyncNotes")
end)
...
RegisterNetEvent("legends-notes:client:SyncNotes", function(newNotes)
    if not newNotes then
        local p = promise.new()

        RSGCore.Functions.TriggerCallback('legends-notes:server:SyncNotes', function(cb)
            p:resolve(cb)
        end)
    ...
    for k, v in pairs(Notes) do
        if not Props[k] and Notes[k] then
            ...
            Targets[k] = exports['legends-target']:AddCircleZone(k, vector3(v.coords.x, v.coords.y, v.coords.z-1.1), 0.5,
            {
                name            = k,
            ...
            },
            {
                options =
                {
                    {
                        type    = "server",
                        event   = "legends-notes:server:ReadNote",
                        icon    = "fas fa-receipt",
    ...
    for k in pairs(Props) do
        if not Notes[k] then
            exports["legends-target"]:RemoveZone(k)
...
RegisterNetEvent("legends-notes:client:CreateNote", function()
    ...
    if dialog.note then
        ...
        TriggerServerEvent("legends-notes:server:CreateNote",
        {
...
RegisterNetEvent("legends-notes:client:ReadNote", function(data)
    local notepad = {}
    ...
    notepad[#notepad + 1] =
    {
        ...
        params                  =
        {
            event               = "legends-notes:client:DestroyNote",
            args                = data
        }
    }

    exports["legends-menu"]:openMenu(notepad)
end)

RegisterNetEvent("legends-notes:client:DestroyNote", function(data)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent("legends-notes:server:DestroyNote", data.id)
end)
