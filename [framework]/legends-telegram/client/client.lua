...
Citizen.CreateThread(function()
    for i = 1, #Config.PostOfficeLocations do
        ...
        exports['legends-core']:createPrompt(pos.location, pos.coords, RSGCore.Shared.Keybinds['J'], 'Open ' .. pos.name, {
            type = 'client',
            event = 'legends-telegram:client:TelegramMenu'
        })
...
-- Telegram Menu
RegisterNetEvent('legends-telegram:client:TelegramMenu', function(data)
    exports['legends-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:ReadMessages',
                isServer = false
            }
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:WriteMessagePostOffice',
                isServer = false
            }
        },
        {
...
-- Write Message
RegisterNetEvent('legends-telegram:client:WriteMessagePostOffice', function()
    RSGCore.Functions.TriggerCallback('legends-telegram:server:GetPlayersPostOffice', function(players)
        local option = {}
        ...
        if players~=nil then
            ...
            local input = exports['legends-input']:ShowInput({
            ...
            if input ~= nil then
                ...
                TriggerServerEvent('legends-telegram:server:SendMessagePostOffice', sendertelegram, senderfullname, input.recipient, input.subject, input.message)
            end
        else
...
local function Prompts()
    ...
    TriggerEvent("legends-telegram:client:ReadMessages")

    TriggerServerEvent('legends-telegram:server:DeliverySuccess', sID, tPName)
...
-- Receive Message
RegisterNetEvent('legends-telegram:client:ReceiveMessage')
AddEventHandler('legends-telegram:client:ReceiveMessage', function(SsID, StPName)
    isReceiving = true
    sID = SsID
    tPName = StPName
    local ped = PlayerPedId()
...
-- Write the Message
RegisterNetEvent('legends-telegram:client:WriteMessage', function()
    RSGCore.Functions.TriggerCallback('legends-telegram:server:GetPlayers', function(players)
        if players ~= nil then
            local citizenid = 0
            local name = 0
            ...
            TriggerServerEvent('legends-telegram:server:SendMessage', senderID, sendertelegram, senderfullname, input.recipient, Lang:t('desc.message_prefix')..': '..input.subject, input.message)
        else
            RSGCore.Functions.Notify("You Need To Add People to Your Addressbook", 'error')
        end
    end)
...
-- Read the Message
RegisterNetEvent('legends-telegram:client:ReadMessages')
AddEventHandler('legends-telegram:client:ReadMessages', function()
    InMenu = true
    ...
    TriggerServerEvent('legends-telegram:server:CheckInbox')
end)

-- Show Messages List
RegisterNetEvent('legends-telegram:client:InboxList')
AddEventHandler('legends-telegram:client:InboxList', function(data)
    SendNUIMessage
    ({
...
RegisterNUICallback('getview', function(data)
    TriggerServerEvent('legends-telegram:server:GetMessages', tonumber(data.id))
end)

-- Message Data
RegisterNetEvent('legends-telegram:client:MessageData')
AddEventHandler('legends-telegram:client:MessageData', function(tele)
    SendNUIMessage
    ({
        type = 'view',
        telegram = tele
...
RegisterNUICallback('delete', function(data)
    TriggerServerEvent('legends-telegram:server:DeleteMessage', tonumber(data.id))
end)
...
AddEventHandler("onResourceStop", function(resourceName)
    ...
    for i = 1, #Config.PostOfficeLocations do
        ...
        exports['legends-core']:deletePrompt(pos.location)
    end
...
-- AddressBook
RegisterNetEvent('legends-telegram:client:OpenAddressbook', function()
    exports['legends-menu']:openMenu({
        {
            header = "| Address Book |",
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:ViewAddressBook',
                isServer = false
            }
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:AddPersonMenu',
                isServer = false
            }
        },
        {
            ...
            params = {
                event = 'legends-telegram:client:RemovePersonMenu',
                isServer = false
            }
        },
        {
...
RegisterNetEvent('legends-telegram:client:AddPersonMenu', function()
    local input = exports['legends-input']:ShowInput({
        header = "Add New Person",
        submitText = "Submit",
            inputs = {
        ...
        if input ~= nil then
            TriggerServerEvent('legends-telegram:server:SavePerson', input.name, input.cid)
        end
end)
RegisterNetEvent('legends-telegram:client:ViewAddressBook', function()
    RSGCore.Functions.TriggerCallback('legends-telegram:server:GetPlayers', function(players)
        if players ~= nil then
            local options = {
                {
            ...
            exports['legends-menu']:openMenu(options)
        else
            RSGCore.Functions.Notify("You Need To Add People to Your Addressbook", 'error')
        end
    end)
...
RegisterNetEvent('legends-telegram:client:RemovePersonMenu', function()
    RSGCore.Functions.TriggerCallback('legends-telegram:server:GetPlayers', function(players)
        if players ~= nil then
            local option = {}
            for i = 1, #players do
            ...
            if input ~= nil then
                TriggerServerEvent('legends-telegram:server:RemovePerson', input.citizenid)
            end
        else
            RSGCore.Functions.Notify("You Need To Add People to Your Addressbook", 'error')
        end
